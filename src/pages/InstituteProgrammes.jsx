...
const InstituteProgrammes = () => {
  ...
  const fetchData = async () => {
    setFetching(true);
    try {
      // Fetch Institutions for the dropdown list
      ...
      // Fetch Programmes
      const progRes = await fetch(`${import.meta.env.VITE_API_URL}/institution/programmes/getall`);
      if (progRes.ok) {
  ...
  const handleOpenModal = (item = null) => {
    if (item) {
      // Resolve the institution ID string if populated as an object
      ...
      setFormData({ institutionId: '', name: '', shortName: '', overview: '', duration: '', eligibility: '', careerPath: '' });
    }
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = formData._id 
        ? `${import.meta.env.VITE_API_URL}/institution/programmes/update/${formData._id}` 
        : `${import.meta.env.VITE_API_URL}/institution/programmes/create`;
  ...
  const confirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/institution/programmes/delete/${deleteConfirmId}`, {
        method: 'DELETE',
      });
  ...
  return (
    <div>
      <InstitutionHeader 
        ...
        subtitle="Manage academic programmes and curriculum paths for Institutions."
...
export default InstituteProgrammes;
